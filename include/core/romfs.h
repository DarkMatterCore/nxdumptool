...
NX_INLINE bool romfsIsValidContext(const RomFileSystemContext *ctx)
{
...
NX_INLINE const void *romfsGetEntryByOffset(const RomFileSystemContext *ctx, void *entry_table, u64 entry_table_size, u64 entry_size, u64 entry_offset)
{
    if (!romfsIsValidContext(ctx) || !entry_table || !entry_table_size || !entry_size || (entry_offset + entry_size) > entry_table_size) return NULL;
    return ((const u8*)entry_table + entry_offset);
}

NX_INLINE const RomFileSystemDirectoryEntry *romfsGetDirectoryEntryByOffset(const RomFileSystemContext *ctx, u64 dir_entry_offset)
{
    return (ctx ? (const RomFileSystemDirectoryEntry*)romfsGetEntryByOffset(ctx, ctx->dir_table, ctx->dir_table_size, sizeof(RomFileSystemDirectoryEntry), dir_entry_offset) : NULL);
}

NX_INLINE const RomFileSystemFileEntry *romfsGetFileEntryByOffset(const RomFileSystemContext *ctx, u64 file_entry_offset)
{
    return (ctx ? (const RomFileSystemFileEntry*)romfsGetEntryByOffset(ctx, ctx->file_table, ctx->file_table_size, sizeof(RomFileSystemFileEntry), file_entry_offset) : NULL);
}
