...
NX_INLINE bool pfsIsValidContext(const PartitionFileSystemContext *ctx)
{
...
NX_INLINE u32 pfsGetEntryCount(const PartitionFileSystemContext *ctx)
{
    return (pfsIsValidContext(ctx) ? ((const PartitionFileSystemHeader*)ctx->header)->entry_count : 0);
}

NX_INLINE const PartitionFileSystemEntry *pfsGetEntryByIndex(const PartitionFileSystemContext *ctx, u32 idx)
{
    return (idx < pfsGetEntryCount(ctx) ? (const PartitionFileSystemEntry*)(ctx->header + sizeof(PartitionFileSystemHeader) + (idx * sizeof(PartitionFileSystemEntry))) : NULL);
}

NX_INLINE const char *pfsGetNameTable(const PartitionFileSystemContext *ctx)
{
    u32 entry_count = pfsGetEntryCount(ctx);
    return (entry_count ? (const char*)(ctx->header + sizeof(PartitionFileSystemHeader) + (entry_count * sizeof(PartitionFileSystemEntry))) : NULL);
}

NX_INLINE u32 pfsGetNameTableSize(const PartitionFileSystemContext *ctx)
{
    return (pfsIsValidContext(ctx) ? ((const PartitionFileSystemHeader*)ctx->header)->name_table_size : 0);
}

NX_INLINE const char *pfsGetEntryName(const PartitionFileSystemContext *ctx, const PartitionFileSystemEntry *fs_entry)
{
    const char *name_table = pfsGetNameTable(ctx);
    if (!name_table || !fs_entry || fs_entry->name_offset >= pfsGetNameTableSize(ctx) || !name_table[fs_entry->name_offset]) return NULL;
    return (name_table + fs_entry->name_offset);
}

NX_INLINE const char *pfsGetEntryNameByIndex(const PartitionFileSystemContext *ctx, u32 idx)
{
    const PartitionFileSystemEntry *fs_entry = pfsGetEntryByIndex(ctx, idx);
    const char *name_table = pfsGetNameTable(ctx);
    return ((fs_entry && name_table) ? (name_table + fs_entry->name_offset) : NULL);
}

NX_INLINE const PartitionFileSystemEntry *pfsGetEntryByName(const PartitionFileSystemContext *ctx, const char *name)
{
    u32 idx = 0;
    return (pfsGetEntryIndexByName(ctx, name, &idx) ? pfsGetEntryByIndex(ctx, idx) : NULL);
}

NX_INLINE void pfsWriteEntryPatchToMemoryBuffer(const PartitionFileSystemContext *ctx, NcaHierarchicalSha256Patch *patch, void *buf, u64 buf_size, u64 buf_offset)
{
...
NX_INLINE u32 pfsGetEntryCountFromImageContext(const PartitionFileSystemImageContext *ctx)
{
    return (ctx ? ctx->header.entry_count : 0);
}

NX_INLINE const PartitionFileSystemEntry *pfsGetEntryByIndexFromImageContext(const PartitionFileSystemImageContext *ctx, u32 idx)
{
    return (idx < pfsGetEntryCountFromImageContext(ctx) ? &(ctx->entries[idx]) : NULL);
}

NX_INLINE const char *pfsGetEntryNameByIndexFromImageContext(const PartitionFileSystemImageContext *ctx, u32 idx)
{
    const PartitionFileSystemEntry *fs_entry = pfsGetEntryByIndexFromImageContext(ctx, idx);
